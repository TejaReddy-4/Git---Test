...
StartProxy.TeamMember(driver, "Tim Wynn (100341987)");

task = Task.EnterTask(driver, "Hire Team Member");

SupervisoryOrganization(driver, "Tim Wynn", "BWW_0685_AGM_Team (Tim Wynn (100341987))");

input(driver, "Hire", "Pre-Hire=Aaron Denicola");
Employee="Aaron Denicola";                                                                                                                                                                         driver.findElement(By.xpath("//*[@title = 'OK']")).click();

Calendar.Date(driver, "Hire Date", "April 2020", "17");

input(driver, "Hire", "Reason=New Hire");

input(driver, "Job Details", "Team Member Type=Regular", "Job Profile=Bartender", 
		      "Time Type=Part time", "Location=BWW 0685 Leavenworth, KS");                                                                                                                         Thread.sleep(2000); driver.findElement(By.xpath("//*[@title = 'Submit']")).click(); wait.until(ExpectedConditions.invisibilityOf(driver.findElement(By.xpath("//*[@title = 'Submit']")))); 

inbox(driver, "Cost Center=31000685 LEAVENWORTH, KS", "Hourly:Amount=8.00",
		//"User Name=jKiara4", "New Password=KiaraTest-4", "Verify New Password=KiaraTest-4", 
		"Date of Birth=12/25/1989", "Select a veteran status=I AM NOT A VETERAN", 
		"Legal Name:First Name=Aaron&Last Name=Bennett", "Relationship:Relationship=Other", "Primary Phone:Phone Number=7245892136",
		"Proposed IDs:Country=United States of America&National ID Type=SSN&Add/Edit ID=Random&Issued Date=07/10/2000&Expiration Date=10/12/2020");
	...
	WebDriverWait wait = new WebDriverWait(driver, 5);
		...
		if(Enter.length == 0) {}
			   ...
			   for(String i: Enter) {
				   ...
				   if(driver.findElements(By.xpath("//label[text()='"+field+"']//parent::div/following-sibling::div/div/div/div/div/div/div[1]/following-sibling::div/ul/li")).size()>0) {}				  
				   else {
...
//---------------------------------Inbox Tasks--------------------------------------------------
WebDriverWait waitBoxes = new WebDriverWait(driver, 7);
driver.manage().timeouts().implicitlyWait(0, TimeUnit.SECONDS);
...
//$x("//*[contains(text(), 'Allison Hirte') and @data-automation-id='inbox_row_title']")
...
public static void SupervisoryOrganization(WebDriver driver, String Name, String SuperOrg)
	   {
		   WebDriverWait wait = new WebDriverWait(driver, 25);
		   Actions a=new Actions(driver);
		   String selectedItem = driver.findElement(By.xpath("//div[@data-automation-id='selectedItem']")).getText();
		   
		   if(selectedItem.contains(SuperOrg))
		   {   
			 //Submit
		   ...
		   wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[contains(@data-automation-label, '"+SuperOrg+"')]")));
		   a.moveToElement(driver.findElement(By.xpath("//*[contains(@data-automation-label, '"+SuperOrg+"')]"))).click().build().perform();
		   }
	   }  
...
public static void Boxes(WebDriver driver) throws InterruptedException
{
...
//--------------------------------------------------Searchbox-----------------------------------------------
						   if(driver.findElements(By.xpath("//label[text()='"+field+"']//parent::div/following-sibling::div/div/div/div/div/div/div/input")).size()>0) {
							   ...
							   else{driver.findElement(By.xpath("//label[text()='"+field+"']//parent::div/following-sibling::div/div/div/div/div/div/div/input")).click();
...
Thread.sleep(500);
	...
	{	((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", driver.findElement(By.xpath("//*[text()='Done']")));
		...
		Thread.sleep(2000); driver.manage().timeouts().implicitlyWait(500, TimeUnit.MILLISECONDS);}
	     ...
	     if(driver.findElements(By.xpath("//*[contains(text(), 'Submit') and not(contains(@title, 'Actions'))]")).size()>0) {
		   ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", driver.findElement(By.xpath("//*[text()='Submit']")));
		  driver.findElement(By.xpath("//*[text()='Submit']")).click();
		   wait.until(ExpectedConditions.invisibilityOf(driver.findElement(By.xpath("//*[text()='Submit']"))));
		   Thread.sleep(2000); 
